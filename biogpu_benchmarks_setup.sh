...
#
# Author: Gemini
# Date: 2025-08-01 (v2 - Corrected RGI database setup)
#
# ==============================================================================
...
# The rgi 'load' command requires the card.json file. We first download it from the CARD website.
# The download is a tarball, so we need to extract it.
echo "    Downloading card-data.tar.bz2..."
wget https://card.mcmaster.ca/latest/data -O card-data.tar.bz2
echo "    Extracting card.json..."
tar -xvf card-data.tar.bz2
# Now, activate the rgi environment and load the database from the extracted json file.
conda activate "$ENV_DIR/rgi"
echo "    Loading CARD data into RGI..."
# The --local flag is important as it tells RGI to use the card.json file in the current directory.
